...
export default function CmProjectsPage() {
  const cases = useCaseManagerStore((s) => s.cases);
  return (
    <div className="space-y-6">
      <PageHeader
        title="Projects"
        description="Workspaces that organize cases, queues, templates, and automations."
      />
        ...
        {CM_PROJECTS.map((p) => {
          ...
          return (
            <Card key={p.id} className="transition-colors hover:border-primary/40">
              <Link href={`/case-manager/projects/${p.id}`} className="block">
